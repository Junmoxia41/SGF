...
export async function handleGetInvoices(req, res, url) {
    ...
    try {
        ...
        const cnt = await query(`SELECT COUNT(*) AS CNT FROM sgf_facturas ${where}`, params);
        const total = cnt[0]?.CNT || 0;
        const offset = (page - 1) * pageSize;
        const invoices = await query(`SELECT * FROM sgf_facturas ${where} ORDER BY created_at DESC LIMIT :lim OFFSET :off`, { ...params, lim: pageSize, off: offset });
        // Servicios
        const ids = invoices.map((i) => i.ID || i.id);
        let svcMap = {};
        if (ids.length > 0) {
            const ph = ids.map((_, i) => `:id${i}`).join(",");
            const sp = {};
            ids.forEach((id, i) => { sp[`id${i}`] = id; });
            const svcs = await query(`SELECT * FROM sgf_servicios WHERE factura_id IN (${ph}) ORDER BY id`, sp);
            for (const s of svcs) {
        ...
        const result = invoices.map((i) => ({ ...i, servicios: svcMap[i.ID || i.id] || [] }));
        return sendJson(res, 200, { success: true, data: result, meta: { total, page, pageSize, totalPages: Math.ceil(total / pageSize) } });
    }
    catch (err) {
        return sendJson(res, 503, { success: false, error: `Error: ${err.message}` });
    }
}
export async function handleGetInvoice(req, res, invId) {
    try {
        const rows = await query("SELECT * FROM sgf_facturas WHERE id = :id", { id: invId });
        if (rows.length === 0)
        ...
        const svcs = await query("SELECT * FROM sgf_servicios WHERE factura_id = :id ORDER BY id", { id: invId });
...
export async function handleCreateInvoice(req, res) {
    ...
    try {
        await transaction(async (_conn) => {
            await execute(`INSERT INTO sgf_facturas (id, cliente, cuenta, numero_cliente, numero_cuenta, no_factura, fecha_emision, fecha_vencimiento, periodo_consumo, codigo_pago, moneda, nit, cuota, consumo, comision, impuesto, total, total_pagar, estado, parser, ocr_confidence, ocr_duration, archivo, texto_ocr, user_id)
         VALUES (:id, :cl, :cu, :nc, :ncu, :nf, :fe, :fv, :pc, :cp, :mo, :ni, :ct, :cs, :cm, :im, :tt, :tp, :es, :pa, :oc, :od, :ar, :tx, :uid)`, { id, cl: body.cliente || "", cu: body.cuenta || "", nc: body.numeroCliente || "", ncu: body.numeroCuenta || "",
            ...
            for (const s of svcs) {
                await execute("INSERT INTO sgf_servicios (id, factura_id, numero_servicio, cuota, consumo, comision, impuesto, importe) VALUES (:sid, :fid, :ns, :ct, :cs, :cm, :im, :ip)", { sid: randomUUID(), fid: id, ns: s.numeroServicio || "", ct: Number(s.cuota) || 0, cs: Number(s.consumo) || 0,
...
export async function handleUpdateInvoice(req, res, invId) {
    ...
    try {
        await execute(`UPDATE sgf_facturas SET cliente=:cl, cuenta=:cu, numero_cliente=:nc, numero_cuenta=:ncu, no_factura=:nf, periodo_consumo=:pc, codigo_pago=:cp, estado=:es, moneda=:mo, total_pagar=:tp, updated_at=datetime('now') WHERE id=:id`, { id: invId, cl: body.cliente || "", cu: body.cuenta || "", nc: body.numeroCliente || "", ncu: body.numeroCuenta || "",
...
export async function handleDeleteInvoice(req, res, invId) {
    if (!requireAdmin(req, res))
        return;
    try {
...
export async function handleInvoiceStats(req, res) {
    try {
        const stats = await query(`SELECT COUNT(*) AS TOTAL, SUM(CASE WHEN estado='procesado' THEN 1 ELSE 0 END) AS PROCESADOS,
