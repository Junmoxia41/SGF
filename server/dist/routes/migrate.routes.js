...
export async function handleMigrateFromSqlite(req, res) {
    ...
    await auditLog(req, "migrar_sqlite", "sistema", targetMode, "warn", `Migracion SQLite -> ${targetMode}: usuarios=${copied.usuarios}, sesiones=${copied.sesiones}, facturas=${copied.facturas}, servicios=${copied.servicios}, logs=${copied.logs}, errores=${errors.length}`);
    return sendJson(res, 200, {
...
export async function handleMigrateStatus(req, res) {
    if (!requireAdmin(req, res))
        return;
        ...
        try {
            ...
            const collect = (table) => {
                try {
                    ...
                    let count = 0;
                    if (stmt.step())
                        count = Number(stmt.getAsObject().C ?? 0);
                    stmt.free();
                    return count;
                }
                catch {
