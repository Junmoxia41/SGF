...
export async function handleCreateUser(req, res) {
    ...
    try {
        const exists = await query(`SELECT ID FROM SGF_USUARIOS WHERE USERNAME = :u`, { u: username });
        if (exists.length > 0) {
        ...
        if (getDbMode() === "oracle") {
         ...
         VALUES (:id, :u, :n, :r, :h, 1, SYSTIMESTAMP, SYSTIMESTAMP)`, { id, u: username, n: name, r: role, h: passwordHash });
        }
        else if (getDbMode() === "mssql") {
            await execute(`INSERT INTO SGF_USUARIOS (ID, USERNAME, NAME, ROLE, PASSWORD_HASH, ACTIVE, CREATED_AT, UPDATED_AT)
         VALUES (:id, :u, :n, :r, :h, 1, SYSUTCDATETIME(), SYSUTCDATETIME())`, { id, u: username, n: name, r: role, h: passwordHash });
        }
        else {
            await execute(`INSERT INTO SGF_USUARIOS (ID, USERNAME, NOMBRE, ROL, PASSWORD_HASH, ACTIVO, CREADO)
         VALUES (:id, :u, :n, :r, :h, 1, datetime('now'))`, { id, u: username, n: name, r: role, h: passwordHash });
        }
...
export async function handleUpdateUser(req, res, userId) {
    if (!requireAdmin(req, res))
        return;
    let body;
    try {
...
export async function handleDeleteUser(req, res, userId) {
    if (!requireAdmin(req, res))
        return;
