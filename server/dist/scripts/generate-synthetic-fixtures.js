...
function service(num, cuota, consumo, comision = 0, impuesto = 0) {
    return {
        numero: num,
        cuota,
        consumo,
...
function totals(services, descuento = 0) {
...
function buildColumnarText(data) {
...
function makeManyServices(prefix, count) {
    const services = [];
...
function buildIncompleteText(data) {
...
function buildDamagedText(data) {
    return [
        `Cliente:${data.cliente}`,
        `No. factura: ${data.noFactura}`,
        `Moneda: ${data.moneda}`,
...
function main() {
    ...
    const fixtures = [
        ...
        },
        {
            ...
            content: mutateNoise(buildColumnarText({
                ...base,
...
main();
//# sourceMappingURL=generate-synthetic-fixtures.js.map