...
function normalizeInvoice(row: any) {
  ...
  return {
    ...
    servicios_count:
      row?.SERVICIOS_COUNT !== undefined || row?.servicios_count !== undefined
...
function buildFilters(req: AuthenticatedRequest, url: URL) {
...
function normalizeIncomingServices(body: any) {
  ...
  return incoming
    ...
    .filter((item: any) => item.numero_servicio);
}
...
export async function handleGetFacturas(req: AuthenticatedRequest, res: ServerResponse, url: URL) {
...
export async function handleGetFactura(req: AuthenticatedRequest, res: ServerResponse, facturaId: string) {
  try {
    const rows = await query<any>(`SELECT * FROM SGF_FACTURAS WHERE ID = :id`, { id: facturaId });
    ...
    const servicios = await query<any>(`SELECT * FROM SGF_SERVICIOS WHERE FACTURA_ID = :id ORDER BY NUMERO_SERVICIO`, { id: facturaId });
    return sendJson(res, 200, {
      success: true,
      data: {
        ...factura,
...
export async function handleCreateFactura(req: AuthenticatedRequest, res: ServerResponse) {
  let body: any;
  try {
...
export async function handleUpdateFactura(req: AuthenticatedRequest, res: ServerResponse, facturaId: string) {
  ...
  try {
    const currentRows = await query<any>(`SELECT * FROM SGF_FACTURAS WHERE ID = :id`, { id: facturaId });
    if (currentRows.length === 0) return sendJson(res, 404, { success: false, error: "Factura no encontrada." });
    const current = normalizeInvoice(currentRows[0]);

    const servicios = normalizeIncomingServices(body);
    const cuotaServicios = servicios.reduce((sum: number, item: { cuota: number }) => sum + item.cuota, 0);
    const consumoServicios = servicios.reduce((sum: number, item: { consumo: number }) => sum + item.consumo, 0);
    const comisionServicios = servicios.reduce((sum: number, item: { comision: number }) => sum + item.comision, 0);
    const impuestoServicios = servicios.reduce((sum: number, item: { impuesto: number }) => sum + item.impuesto, 0);
    const importeServicios = servicios.reduce((sum: number, item: { importe: number }) => sum + item.importe, 0);
...
export async function handleDeleteFactura(req: AuthenticatedRequest, res: ServerResponse, facturaId: string) {
  if (!requireAdmin(req, res)) return;

  try {
    const rows = await query<any>(`SELECT ID, NO_FACTURA FROM SGF_FACTURAS WHERE ID = :id`, { id: facturaId });
    if (rows.length === 0) return sendJson(res, 404, { success: false, error: "Factura no encontrada." });
...
export async function handleFacturaStats(req: AuthenticatedRequest, res: ServerResponse) {
  try {
    ...
    const stats = await query<any>(totalQuery);
    const services = await query<any>(`SELECT COUNT(*) AS TOTAL_SERVICIOS FROM SGF_SERVICIOS`);
