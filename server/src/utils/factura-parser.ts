...
export interface StoredParserMeta {
  ...
  requiresReview?: boolean;
  diagnostics?: ParserDiagnostic[];
  strategies?: { services: string; totals: string };
}

const META_PREFIX = "__SGF_META__";
const META_SEPARATOR = "\n__SGF_TEXT__\n";

export function packStoredFacturaText(rawText: string, meta: StoredParserMeta) {
  return `${META_PREFIX}${JSON.stringify(meta)}${META_SEPARATOR}${rawText}`;
}

export function unpackStoredFacturaText(value: string | null | undefined) {
  const text = String(value || "");
  if (!text.startsWith(META_PREFIX)) {
    return { meta: null as StoredParserMeta | null, text };
  }
  ...
  try {
    return { meta: JSON.parse(rawMeta) as StoredParserMeta, text: rawText };
  } catch {
    return { meta: null as StoredParserMeta | null, text: rawText };
  }
}
type Totales = FacturaExtraida["totales"];
...
function amountToken(token: string) {
  return /^-?\d[\d.,]*$/.test(token);
}

function moneyLikeToken(token: string) {
  return /^-?\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})$/.test(token) || /^-?\d+(?:[.,]\d{2})$/.test(token);
}

function amountValue(token: string) {
  return formatearNumero(normalizarNumero(token));
}

function isServiceNumber(value: string) {
  return /^\d{7,10}$/.test(value);
}

function parseRowLikeLine(line: string): ServicioExtraido | null {
  const tokens = cleanLine(line).split(/\s+/).filter(Boolean);
  const numericTokens = tokens.filter(amountToken);
  if (numericTokens.length < 6) return null;
  ...
  for (let serviceParts = 1; serviceParts <= Math.min(3, numericTokens.length - 5); serviceParts += 1) {
    ...
    if (!isServiceNumber(numero)) continue;
...
function getHeaderSection(lines: string[]) {
...
function getServiceSection(lines: string[]) {
...
function parseColumnarSection(sectionLines: string[], diagnostics: ParserDiagnostic[]) {
  ...
  const result: ServicioExtraido[] = [];
  const seen = new Set<string>();
  for (let i = 0; i < max; i += 1) {
    const numero = (columns.servicios[i] || "").replace(/\D/g, "");
    if (!isServiceNumber(numero) || seen.has(numero)) continue;
    seen.add(numero);
    result.push({
      numero,
...
function parseRowSection(sectionLines: string[], diagnostics: ParserDiagnostic[]) {
  const result: ServicioExtraido[] = [];
  const seen = new Set<string>();
  for (const line of sectionLines) {
    const parsed = parseRowLikeLine(line);
    if (!parsed) continue;
    if (seen.has(parsed.numero)) continue;
...
function sumServices(services: ServicioExtraido[]) {
...
function scoreServices(services: ServicioExtraido[]) {
...
function chooseBestServices(sectionLines: string[], diagnostics: ParserDiagnostic[]) {
  const columnarDiagnostics: ParserDiagnostic[] = [];
  const rowDiagnostics: ParserDiagnostic[] = [];
...
function parseHeaderTotals(lines: string[]) {
...
function parseServiceSummaryTotal(sectionLines: string[]) {
  ...
  if (values.length < 5 || !values.slice(0, 5).every((v) => moneyLikeToken(v))) return null;
  return {
    cuota: formatearNumero(normalizarNumero(values[0] || "0")),
    consumo: formatearNumero(normalizarNumero(values[1] || "0")),
    comision: formatearNumero(normalizarNumero(values[2] || "0")),
...
function parseDiscount(lines: string[]) {
...
function chooseTotals(lines: string[], services: ServicioExtraido[], diagnostics: ParserDiagnostic[]) {
...
function computeReviewState(data: Omit<FacturaExtraida, "confidence" | "reviewStatus" | "reviewLabel" | "requiresReview">, confidence: number) {
  ...
  if (errors > 0 || data.servicios.length === 0 || confidence < 70 || missingCritical >= 2) {
    return {
      reviewStatus: "doubtful" as const,
  ...
  return {
    reviewStatus: "high" as const,
    reviewLabel: "Alta confianza",
    requiresReview: false,
  };
}

function computeConfidence(factura: Omit<FacturaExtraida, "confidence" | "reviewStatus" | "reviewLabel" | "requiresReview">) {
  let score = 30;
  if (factura.cliente) score += 10;
  if (factura.noFactura) score += 10;
  if (factura.numeroCliente) score += 6;
  if (factura.numeroCuenta) score += 6;
  if (factura.fechaEmision) score += 4;
  if (factura.fechaVencimiento) score += 4;
  if (factura.periodoConsumo) score += 5;
  if (factura.codigoPago) score += 7;
  if (factura.nit) score += 5;
  score += Math.min(factura.servicios.length, 25);
  if (normalizarNumero(factura.totales.totalPagar) > 0) score += 6;
...
export function parseFacturaEtcsa(text: string): FacturaExtraida {
  const diagnostics: ParserDiagnostic[] = [];
  const lines = getLines(text);
  const headerSection = getHeaderSection(lines);
  const serviceSection = getServiceSection(lines);

  if (!headerSection) diagnostics.push({ level: "error", code: "header-empty", message: "No se pudo detectar la cabecera de la factura." });
  if (!serviceSection.length) diagnostics.push({ level: "warn", code: "service-section-empty", message: "No se detecto claramente la seccion 'Resumen por Servicios'." });
  ...
  if (!cliente) diagnostics.push({ level: "warn", code: "field-cliente", message: "No se pudo extraer el cliente." });
  if (!numeroCliente) diagnostics.push({ level: "warn", code: "field-numero-cliente", message: "No se pudo extraer el numero de cliente." });
  if (!numeroCuenta) diagnostics.push({ level: "warn", code: "field-numero-cuenta", message: "No se pudo extraer el numero de cuenta." });
  if (!noFactura) diagnostics.push({ level: "warn", code: "field-no-factura", message: "No se pudo extraer el numero de factura." });
  if (!fechaEmision) diagnostics.push({ level: "warn", code: "field-fecha-emision", message: "No se pudo extraer la fecha de emision." });
  if (!fechaVencimiento) diagnostics.push({ level: "warn", code: "field-fecha-vencimiento", message: "No se pudo extraer la fecha de vencimiento." });
  if (!periodoConsumo) diagnostics.push({ level: "warn", code: "field-periodo", message: "No se pudo extraer el periodo de consumo." });
  if (!codigoPago) diagnostics.push({ level: "warn", code: "field-codigo-pago", message: "No se pudo extraer el codigo de pago." });
  if (!nit) diagnostics.push({ level: "warn", code: "field-nit", message: "No se pudo extraer el NIT." });
  ...
  const facturaBase: Omit<FacturaExtraida, "confidence" | "reviewStatus" | "reviewLabel" | "requiresReview"> = {
    cliente,
    cuenta,
    direccion,
    numeroCliente,
